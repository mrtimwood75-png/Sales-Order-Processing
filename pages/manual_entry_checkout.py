...
def directsms_send_message(to_mobile, message, debug=False):
    ...
    message_type_param = get_secret("DIRECTSMS_MESSAGE_TYPE_PARAM")
    message_type_value = get_secret("DIRECTSMS_MESSAGE_TYPE_VALUE")

    if message_type_param and message_type_value:
        data[str(message_type_param)] = str(message_type_value)
...
with st.form("manual_entry_form"):
    ...
    balance_due = col_i.text_input("Balance due", value=f"{float(st.session_state['manual_balance_due']):.2f}", disabled=True)
    payment_amount_input = col_j.text_input("Payment amount", value=f"{float(st.session_state['manual_payment_amount']):.2f}", disabled=True)
...
with sms_col1:
    send_sms_clicked = st.button("Send SMS to Customer", use_container_width=True)
